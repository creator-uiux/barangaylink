...
$concernsStmt = $conn->prepare("SELECT COUNT(*) as total, 
    SUM(CASE WHEN status = 'pending' THEN 1 ELSE 0 END) as pending 
    FROM concerns WHERE user_id = ?");
$concernsStmt->bind_param("i", $userId);
$concernsStmt->execute();
$concernsData = $concernsStmt->get_result()->fetch_assoc();

// Get user's document requests count
$docsStmt = $conn->prepare("SELECT COUNT(*) as total, 
    SUM(CASE WHEN status = 'pending' OR status = 'processing' THEN 1 ELSE 0 END) as pending 
    FROM documents WHERE user_id = ?");
$docsStmt->bind_param("i", $userId);
$docsStmt->execute();
$docsData = $docsStmt->get_result()->fetch_assoc();

// Get recent activities (concerns + documents)
$recentConcerns = $conn->prepare("SELECT id, category as title, status, created_at, 'concern' as type FROM concerns WHERE user_id = ? ORDER BY created_at DESC LIMIT 5");
$recentConcerns->bind_param("i", $userId);
$recentConcerns->execute();
$concernsList = $recentConcerns->get_result()->fetch_all(MYSQLI_ASSOC);

$recentDocs = $conn->prepare("SELECT id, document_type as title, status, created_at, 'document' as type FROM documents WHERE user_id = ? ORDER BY created_at DESC LIMIT 5");
$recentDocs->bind_param("i", $userId);
$recentDocs->execute();
$docsList = $recentDocs->get_result()->fetch_all(MYSQLI_ASSOC);
...
<div class="space-y-6 sm:space-y-8">
                ...
                <div class="bg-white/10 backdrop-blur-lg rounded-xl lg:rounded-2xl p-3 sm:p-4 border border-white/20 sm:col-span-2 lg:col-span-1">
                    <div class="flex items-center space-x-2 sm:space-x-3">
                        ...
                        <div>
                            ...
                            <p class="text-purple-300 text-xs sm:text-sm"><?php echo ($concernsData['pending'] + $docsData['pending']); ?> pending</p>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
        ...
        <div class="group bg-white/80 backdrop-blur-lg rounded-2xl p-6 border border-gray-200/50 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105">
            ...
            <div class="mb-3">
                <p class="text-4xl font-bold text-gray-800 mb-1"><?php echo $concernsData['total']; ?></p>
                <p class="text-sm text-gray-600"><?php echo $concernsData['pending']; ?> pending</p>
            </div>
        ...
        <div class="group bg-white/80 backdrop-blur-lg rounded-2xl p-6 border border-gray-200/50 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105">
            ...
            <div class="mb-3">
                <p class="text-4xl font-bold text-gray-800 mb-1"><?php echo $docsData['total']; ?></p>
                <p class="text-sm text-gray-600"><?php echo $docsData['pending']; ?> processing</p>
            </div>
            <div class="flex items-center space-x-2">
        ...
        <div class="space-y-6">
            ...
            <?php else: ?>
                <?php foreach ($activities as $activity): ?>
                    <div class="flex items-center justify-between p-4 bg-white/50 rounded-2xl border border-gray-200/50 hover:shadow-md transition-all duration-200 group">
    ...
    <div class="bg-gradient-to-br from-gray-50 to-blue-50 rounded-3xl p-8 border border-gray-200/50">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            <!-- Submit Concern -->
            <a href="?view=submit-concern" class="group bg-white/80 backdrop-blur-lg rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-gray-200/50 hover:shadow-xl transition-all duration-300 transform hover:scale-105 text-left block">
            ...
            <!-- Request Document -->
            <a href="?view=document-request" class="group bg-white/80 backdrop-blur-lg rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-gray-200/50 hover:shadow-xl transition-all duration-300 transform hover:scale-105 text-left block">
            ...
            <!-- Community Directory -->
            <a href="?view=community-directory" class="group bg-white/80 backdrop-blur-lg rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-gray-200/50 hover:shadow-xl transition-all duration-300 transform hover:scale-105 text-left block">
            ...
            <!-- Emergency Alerts -->
            <a href="?view=emergency-alerts" class="group bg-white/80 backdrop-blur-lg rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-gray-200/50 hover:shadow-xl transition-all duration-300 transform hover:scale-105 text-left block">
